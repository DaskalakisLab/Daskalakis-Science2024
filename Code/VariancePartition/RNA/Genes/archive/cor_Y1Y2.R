...
cor_Y1Y2 <- function(info, form, out_file, multi_region=F){
  if (multi_region){
    info$mPFC <- 0
...
for (f in info_files){
  ...
  if ((dx == "All" & region == "All")){
    ...
    out_ancestry_est <- paste0("/data/humgen/daskalakislab/dipietro/SciencePaper/Data/RNA/VariancePartition/CorFiles/Y1Y2/ancestry_est_",dx, "_", region, ".RDS")
    out_ancestry_pc <- paste0("/data/humgen/daskalakislab/dipietro/SciencePaper/Data/RNA/VariancePartition/CorFiles/Y1Y2/ancestry_pc_",dx, "_", region, ".RDS")
    out_genetic_pc <- paste0("/data/humgen/daskalakislab/dipietro/SciencePaper/Data/RNA/VariancePartition/CorFiles/Y1Y2/genetic_pc_",dx, "_", region, ".RDS")
    ...
    if (!file.exists(out_ancestry_est)){
      cor_Y1Y2(info = info, form = form_ancestry_est_all_all, out_file = out_ancestry_est, multi_region = T)
    }
    if (!file.exists(out_ancestry_pc)){
      cor_Y1Y2(info = info, form = form_ancestry_pc_all_all, out_file = out_ancestry_pc, multi_region = T)
    }
    if (!file.exists(out_genetic_pc)){
      cor_Y1Y2(info = info, form = form_genetic_pc_all_all, out_file = out_genetic_pc, multi_region = T)
    }
    ...
    print(out_genetic_pc)
    if (!file.exists(out_ancestry_est)){
      cor_Y1Y2(info = info, form = form_ancestry_est_all_split, out_file = out_ancestry_est, multi_region = F)
    }
    if (!file.exists(out_ancestry_pc)){
      cor_Y1Y2(info = info, form = form_ancestry_pc_all_split, out_file = out_ancestry_pc, multi_region = F)
    }
    if (!file.exists(out_genetic_pc)){
      cor_Y1Y2(info = info, form = form_genetic_pc_all_split, out_file = out_genetic_pc, multi_region = F)
    }
    ...
    print(out_genetic_pc)
    if (!file.exists(out_ancestry_est)){
      cor_Y1Y2(info = info, form = form_ancestry_est_ptsd_all, out_file = out_ancestry_est, multi_region = T)
    }
    if (!file.exists(out_ancestry_pc)){
      cor_Y1Y2(info = info, form = form_ancestry_pc_ptsd_all, out_file = out_ancestry_pc, multi_region = T)
    }
    if (!file.exists(out_genetic_pc)){
      cor_Y1Y2(info = info, form = form_genetic_pc_ptsd_all, out_file = out_genetic_pc, multi_region = T)
    }
    ...
    if (!file.exists(out_ancestry_est)){
      cor_Y1Y2(info = info, form = form_ancestry_est_mdd_all, out_file = out_ancestry_est, multi_region = T)
    }
    if (!file.exists(out_ancestry_pc)){
      cor_Y1Y2(info = info, form = form_ancestry_pc_mdd_all, out_file = out_ancestry_pc, multi_region = T)
    }
    if (!file.exists(out_genetic_pc)){
      cor_Y1Y2(info = info, form = form_genetic_pc_mdd_all, out_file = out_genetic_pc, multi_region = T)
    }
    ...
    if (!file.exists(out_ancestry_est)){
      cor_Y1Y2(info = info, form = form_ancestry_est_ptsd_split, out_file = out_ancestry_est, multi_region = F)
    }
    if (!file.exists(out_ancestry_pc)){
      cor_Y1Y2(info = info, form = form_ancestry_pc_ptsd_split, out_file = out_ancestry_pc, multi_region = F)
    }
    if (!file.exists(out_genetic_pc)){
      cor_Y1Y2(info = info, form = form_genetic_pc_ptsd_split, out_file = out_genetic_pc, multi_region = F)
    }
    ...
    print(out_genetic_pc)
    if (!file.exists(out_ancestry_est)){
      cor_Y1Y2(info = info, form = form_ancestry_est_mdd_split, out_file = out_ancestry_est, multi_region = F)
    }
    if (!file.exists(out_ancestry_pc)){
      cor_Y1Y2(info = info, form = form_ancestry_pc_mdd_split, out_file = out_ancestry_pc, multi_region = F)
    }
    if (!file.exists(out_genetic_pc)){
      cor_Y1Y2(info = info, form = form_genetic_pc_mdd_split, out_file = out_genetic_pc, multi_region = F)
    }
  }
}
