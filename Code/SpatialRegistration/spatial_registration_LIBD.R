# download spatialDLPFC github to run this
# 
library("jaffelab")
library("here")
library("purrr")
...
de_gene <- mget(load("genes.rda", verbose = TRUE))
de_protein <- mget(load("proteins.rda", verbose = TRUE))
...
sigGeneSign[,1] <- gtf_df$gene_id[match(sigGeneSign[,1],gtf_df$gene_name)]
...
sigGeneSign <- data.frame(sigGeneSign)
colnames(sigGeneSign) <- c("gene","change")
ptsd_genes$protein_mPFC[["PTSD_mPFC"]]  <- sigGeneSign
...
bayesSpace_registration <-
  lapply(bayesSpace_registration_fn[-5], function(x) {
    get(load(x))
  })
...
source("spatialDLPFC/code/analysis/10_clinical_gene_set_enrichment/gene_set_enrichment_plot_complex.R")
...
enriched_select <- rbind(
  enriched$gene_DLPFC$k09 %>% filter(ID %in% c("MDD_DLPFC")),
  enriched$gene_mPFC$k09 %>% filter(ID %in% c("MDD_mPFC", "PTSD_mPFC"))
)

gene_list_count_select <- rbind(
  gene_list_count$gene_DLPFC[c("MDD_DLPFC"), , drop = FALSE],
  gene_list_count$gene_mPFC[c("MDD_mPFC", "PTSD_mPFC"), , drop = FALSE]
)

enriched_select <- rbind(enriched$gene_mPFC$k09 %>% filter(ID %in% c("MDD_mPFC", "PTSD_mPFC"))
)
...
pdf(here(dir_plots, "Enrich_Gene_select_k09.pdf"), height = 8, width = 5)
...
enriched_select_ptsd <- list(
  gene = enriched$gene_mPFC$k09 %>% filter(ID %in% c("MDD_mPFC", "PTSD_mPFC")),
  protein = enriched$protein_mPFC$k09 %>% filter(ID %in% c("MDD_mPFC", "PTSD_mPFC"))
)

gene_list_count_select_ptsd <- list(
  gene = gene_list_count$gene_mPFC[c("MDD_mPFC", "PTSD_mPFC"), , drop = FALSE],
  protein = gene_list_count$protein_mPFC[c("MDD_mPFC", "PTSD_mPFC"), , drop = FALSE]
)
