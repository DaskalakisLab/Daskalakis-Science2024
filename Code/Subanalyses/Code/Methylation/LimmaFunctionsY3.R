...
filter_expr <- function(expr, anno){
  ...
  expr <- expr[,names(expr)[(names(expr) %in% sample_ids)]]
  if (setequal(names(expr),sample_ids)){
    print("Expression Data Filtered Succesfully!")
  }else{
    print("Voom object missing sample IDs! Please check your data.")
  }
  return(expr)
}
...
run_limma <- function(anno, expr, covariates, contrast_var, type=NULL){
  ...
  SE_table$meth <- rownames(SE_table)
  
  # Run limma
  results <-limma::topTable(fit2.ebayes,number=nrow(fit2.ebayes),coef=1, adjust="BH")
  results[[type]] <- rownames(results)
  
  results <- map_cpg_names(results)
  
  results <- merge(results, SE_table, by="meth", all.x=T)
...
limma_wrapper <- function(anno_file, expr_file, region, dx=NULL, ptsd_vs_mdd = F, ptsd_mdd = F, sex=NULL, 
                          childtrauma=F, suicide=F,type,
  ...
  anno_br_dx <- anno_br_dx[anno_br_dx$BrNum %in% names(expr_br_dx),]
  rm(expr)
  
  expr_br_dx <- expr_br_dx[,anno_br_dx$BrNum]
...
# expr_file = paste0("/data/humgen/daskalakislab/dipietro/SciencePaper/Data/Methylation/AartiNormalized/",region,"Mfunnorm.RDS")
# region = region
# dx="PTSD"
# ptsd_vs_mdd = F
# ptsd_mdd = F
# type = "meth"
# covariates = c("PTSD","AgeDeath","PMI","Sex","ancestryPC1","ancestryPC2","Meth_PC1","Meth_PC2","Smoking","ex","inhib","Astro","Micro","Oligo","OPC","Tcell") 
# contrast_var = "PTSD"
# out_folder = "/data/humgen/daskalakislab/dipietro/SciencePaper/Data/Methylation/Limma_Results/CTP7/Y1Y2/PTSD/"
